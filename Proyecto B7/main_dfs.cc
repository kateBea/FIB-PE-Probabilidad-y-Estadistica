...
int main(void)
{
    ...
    std::chrono::time_point<std::chrono::high_resolution_clock> dfs_start, dfs_end;
    ...
    double elaps_time_dfs = double(std::chrono::duration_cast<std::chrono::nanoseconds> (dfs_end - dfs_start).count());
    std::time_t end_time = std::chrono::high_resolution_clock::to_time_t(end);
  
    cout << "finished computation at " << std::ctime(&end_time);
    cout << "elapsed time program: " << elaps_time/1e3 << " ms\n";
    cout << "elapsed time dfs_find(): " << elaps_time_dfs/1e3 << " ms\n";
